...
<div class="max-w-6xl mx-auto">
    ...
    <div class="flex flex-col lg:flex-row gap-12">
        ...
        <!-- Form Card -->
        <div class="lg:w-2/3">
            <form action="{{ route('superadmin.admins.store') }}" method="POST" class="ivory-glass p-6 md:p-8 lg:p-12 rounded-[2rem] shadow-2xl shadow-primary/5 space-y-8 border border-outline-variant/10">
                @csrf
                <!-- Personal Details Section -->
                <section class="space-y-6">
                    <div class="flex items-center gap-2 mb-4">
                        <div class="h-px flex-1 bg-outline-variant/30"></div>
                        <span class="text-[9px] md:text-[10px] font-black uppercase tracking-[0.2em] text-outline px-4 whitespace-nowrap">Institutional Identity</span>
                        <div class="h-px flex-1 bg-outline-variant/30"></div>
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-6">
                        <div class="space-y-1.5">
                            <label class="label-md font-black text-[10px] text-primary uppercase tracking-widest ml-1">Full Name</label>
                            <input name="name" class="w-full bg-surface-container-highest border-2 border-transparent focus:border-primary focus:bg-surface-bright transition-all p-4 md:p-5 rounded-2xl outline-none text-on-surface placeholder:text-outline-variant font-bold text-sm md:text-base" placeholder="Dr. Arthur V. Gate" type="text" required/>
                        </div>
                        <div class="space-y-1.5">
                            <label class="label-md font-black text-[10px] text-primary uppercase tracking-widest ml-1">Institutional ID</label>
                            <input name="id_number" class="w-full bg-surface-container-highest border-2 border-transparent focus:border-primary focus:bg-surface-bright transition-all p-4 md:p-5 rounded-2xl outline-none text-on-surface placeholder:text-outline-variant font-bold text-sm md:text-base" placeholder="AC-2024-0000" type="text" required/>
                        </div>
                    </div>
                    <div class="space-y-1.5">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-6">
                        ...
                        <div class="space-y-1.5">
                            <label class="label-md font-black text-[10px] text-primary uppercase tracking-widest ml-1">Initial Password</label>
                            <input name="password" class="w-full bg-surface-container-highest border-2 border-transparent focus:border-primary focus:bg-surface-bright transition-all p-4 md:p-5 rounded-2xl outline-none text-on-surface font-bold text-sm md:text-base" type="password" required/>
                        </div>
                    </div>
                </section>

                <!-- Action Buttons -->
                <div class="flex flex-col sm:flex-row items-center justify-end gap-4 pt-6 border-t border-dashed border-outline-variant/20">
                    <button class="w-full sm:w-auto px-10 py-4 text-[10px] font-black text-outline hover:text-primary hover:bg-surface-container-highest transition-colors rounded-xl uppercase tracking-widest order-2 sm:order-1" type="button">
                        Cancel
                    </button>
                    <button class="w-full sm:w-auto px-12 py-4 signature-texture text-white text-[10px] font-black rounded-xl shadow-xl shadow-primary/20 active:scale-95 transition-all uppercase tracking-widest order-1 sm:order-2" type="submit">
